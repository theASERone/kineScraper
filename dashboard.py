...
df["fecha_hora_fin_dashboard"] = pd.to_datetime(
    df["fecha"] + " " + df["hora_fin"].astype(str).str.strip(),
    errors="coerce",
)

fin_antes_de_inicio = (
    df["fecha_hora_dashboard"].notna()
    & df["fecha_hora_fin_dashboard"].notna()
    & (df["fecha_hora_fin_dashboard"] < df["fecha_hora_dashboard"])
)
df.loc[fin_antes_de_inicio, "fecha_hora_fin_dashboard"] = (
    df.loc[fin_antes_de_inicio, "fecha_hora_fin_dashboard"] + pd.Timedelta(days=1)
)
...
df["rango_horario_dashboard"] = (
    df["hora_dashboard"].fillna("N/D") + " -> " + df["hora_fin_dashboard"].fillna("N/D")
)
